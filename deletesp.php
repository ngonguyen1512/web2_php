<?php 
    ...
    if (isset($_GET['idsp']) && !empty($_GET['idsp'])){           
        $result = mysqli_query ($con, "DELETE FROM `product` WHERE `idsp`= ".$_GET['idsp']);
        if(!$result) {
        ...
        if($error !== false) { ?>
            <div id="error-notify" class="box-content" style="border: 1px solid white; text-align:center; margin: 10%">
                <h1>Thông báo</h1>
                <h4><?= $error ?></h4>
                <a href="index.php?quanly=sanpham">Danh sách tài khoản.</a>
            </div>
        <?php } else { ?>
            <div id="success-notify" class="box-content" style="border: 1px solid white; text-align:center; margin: 10%">
                ...
                <a href="index.php?quanly=sanpham" style="font-size: 18px;">Danh sách tài khoản.</a>
            </div>
        <?php } ?>
    <?php } 
?>